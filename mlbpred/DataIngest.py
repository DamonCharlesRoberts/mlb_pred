...
class Initializer:
    ...
    Methods:
        ...
        exe: Evokes all of the other methods.
    """

    @classmethod
    def exe(cls, path: str) -> None:
        """Initializes all of the necessary tables.

        This method is responsible for initializing all of the necessary
        tables for for the database.
        """
        # Connect to the DuckDB database.
        con = db.connect(path)
        # Initializes the seasons table.
        cls._seasons(con)
        logger.info("Initialized the seasons table.")
        # Initializes the teams table.
        cls._teams(con)
        logger.info("Initialized the teams table.")
        # Initializes the schedule table.
        cls._schedule(con)
        logger.info("Initialized the schedule table.")
        # Initialize the score table.
        cls._score(con)
        logger.info("Initialized the score table.")
        # Close the connection.
        con.close()
        logger.info("Completed the DB initialization.")

    @staticmethod
    def _seasons(con) -> None:
...
class Ingestor:
    ...
    @classmethod
    def exe(cls, path: str) -> None:
        """Ingests the necessary data."""
        # Connect to the database.
        con = db.connect(path)
        # Add to the seasons table if necessary.
        cls._season(con)
        logger.info("Successfully ingested the season data.")
    ...
    def _season(con) -> None:
        ...
        con.execute(
            """
            insert into seasons (
                season_id, has_wildcard, preseason_start, season_start
                , regular_season_start, regular_season_end, season_end
    ...
    def _teams(con) -> None:
        ...
        in each season of the MLB. The team id's pulled from this endpoint
        which is useful to join team names and team abbreviations
        (which are also in the response to this endpoint) into more
        digestible labels for data vizualizations.
        """
        # Check which seasons, we do not have team data for.
